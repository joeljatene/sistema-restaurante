...
if login():
    ...
    # --- ABA 4: FICHAS TÉCNICAS (ATUALIZADA) ---
        ...
        tab_conferir, tab_cadastro, tab_gerenciar = st.tabs(["✅ Conferir Estoque", "📝 Cadastrar Receita", "⚙️ Gerenciar (Editar/Excluir)"])
        ...
        with tab_cadastro:
            ...
            if not df_prod.empty:
                ...
                if c3.button("➕ Adicionar à Lista"):
                    if txt_prato:
                        item = {"cardapio": sel_cardapio, "prato": txt_prato, "ingrediente": ing_selecionado, "qtd_necessaria": qtd_ing}
                ...
                if len(st.session_state['temp_ingredientes']) > 0:
                    ...
                    if col_save.button("💾 SALVAR RECEITA", type="primary"):
        ...
        with tab_gerenciar:
            ...
            if not df_f.empty:
                ...
                if filtro_cardapio != "Todos":
                    view_df = df_f[df_f['cardapio'] == filtro_cardapio]
                else:
                    view_df = df_f
                ...
                with col_del:
                    ...
                    with st.expander("Opções de Exclusão"):
                        ...
                        if st.button("🚨 EXCLUIR PRATO INTEIRO", type="primary"):
                            ...
                            st.session_state['df_fichas'] = novo_df
                            salvar_dados("fichas.csv", novo_df, "Exclusão Prato")
                            st.success(f"Receita de {prato_selecionado} excluída!")
                            st.rerun()
            else:
                st.warning("Nenhuma receita cadastrada.")
